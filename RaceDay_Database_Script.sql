...
VALUES
    ('Busisiwe Zuma', 'busisiwe.organiser@raceday.co.za', 0x01, 0x01, (SELECT RoleId FROM dbo.Role WHERE RoleName = 'Organiser')),
    ('Paballo Masilela', 'paballo.organiser@raceday.co.za', 0x01, 0x01, (SELECT RoleId FROM dbo.Role WHERE RoleName = 'Organiser')),
    ('Bokamoso Mashamba', 'bokamoso.participant@raceday.co.za', 0x01, 0x01, (SELECT RoleId FROM dbo.Role WHERE RoleName = 'Participant')),
    ('Khensani Tibane', 'khensani.participant@raceday.co.za', 0x01, 0x01, (SELECT RoleId FROM dbo.Role WHERE RoleName = 'Participant'));
GO
...
VALUES
    ('Johannesburg City 10K', 'A fast, flat 10km road race through the Joburg CBD.', '2026-11-14 07:00:00', 'Johannesburg, Gauteng', 10.00, 'Run',
        (SELECT UserId FROM dbo.[User] WHERE Email = 'busisiwe.organiser@raceday.co.za')),
    ...
    ('Pretoria Fun Walk', 'Family-friendly community walk for all ages.', '2026-09-20 08:00:00', 'Pretoria, Gauteng', 5.00, 'Walk',
        (SELECT UserId FROM dbo.[User] WHERE Email = 'paballo.organiser@raceday.co.za'));
GO
...
VALUES
    ((SELECT UserId FROM dbo.[User] WHERE Email = 'bokamoso.participant@raceday.co.za'),
    ...
    ((SELECT UserId FROM dbo.[User] WHERE Email = 'khensani.participant@raceday.co.za'),
     (SELECT EventId FROM dbo.Event WHERE Name = 'Johannesburg City 10K'),
     (SELECT CategoryId FROM dbo.Category WHERE Name = 'Senior' AND EventId = (SELECT EventId FROM dbo.Event WHERE Name = 'Johannesburg City 10K'))),
    ((SELECT UserId FROM dbo.[User] WHERE Email = 'paballo.participant@raceday.co.za'),
...
VALUES
    ((SELECT EnrolmentId FROM dbo.Enrolment
        WHERE ParticipantId = (SELECT UserId FROM dbo.[User] WHERE Email = 'paballo.participant@raceday.co.za')
